...
pub fn entity_list_loop(client: proc_mem::Module, server: proc_mem::Module, game: proc_mem::Process) {
	...
	for i in 1..=player_count {
	    ...
	    let player_pawn = game.read_mem::<usize>(entity_controller + offsets::PLAYER_PAWN)
		.expect("failed to read player_pawn");
	    let list_entity = game.read_mem::<usize>(entity_list + ((0x8 * ((player_pawn & 0x7FFF) >> 9) + 16) as usize))
